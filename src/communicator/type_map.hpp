...
template <typename IdType, typename... Types> struct TypeMap {
    using id_type = IdType;
    static constexpr size_t size = sizeof...(Types);
};

template <typename T> using clean_t = typename std::remove_const_t<std::remove_reference_t<T>>;
...
template <typename Target, typename TM> constexpr bool contains_v = get_id<Target, TM> < TM::size;

template <typename IdType, typename... Types> constexpr bool id_valid(size_t id, TypeMap<IdType, Types...>) {
